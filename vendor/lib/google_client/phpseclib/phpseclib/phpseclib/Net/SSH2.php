...
class SSH2
{
    ...
    private function connect()
    {
        ...
        $this->module->last_packet = microtime(true);
    ...
    private function key_exchange($kexinit_payload_server = false)
    {
        ...
        if ($this->encrypt) {
            ...
            switch ($encrypt) {
                case 'chacha20-poly1305@openssh.com':
                    $encryptKeyLength = 32;
                    $this->module->lengthEncrypt = self::encryption_algorithm_to_crypt_instance($encrypt);
                    $this->module->lengthEncrypt->setKey(substr($key, 32, 32));
            }
        ...
        if ($this->decrypt) {
            ...
            switch ($decrypt) {
                case 'chacha20-poly1305@openssh.com':
                    $decryptKeyLength = 32;
                    $this->module->lengthDecrypt = self::encryption_algorithm_to_crypt_instance($decrypt);
                    $this->module->lengthDecrypt->setKey(substr($key, 32, 32));
            }
    ...
    protected function sublogin($username, ...$args)
    {
        ...
        if (empty($args)) {
            return $this->module->login_helper($username);
        }

        foreach ($args as $arg) {
            if ($this->module->login_helper($username, $arg)) {
                return true;
            }
        }
        return false;
    ...
    private function login_helper($username, $password = null)
    {
        ...
        if (!($this->bitmap & self::MASK_LOGIN_REQ)) {
            ...
            if ($response === false) {
                if ($this->retry_connect) {
                    ...
                    return $this->module->login_helper($username, $password);
                }
                $this->disconnect_helper(NET_SSH2_DISCONNECT_CONNECTION_LOST);
                throw new ConnectionClosedException('Connection closed by server');
            }
        ...
        if (strlen($this->module->last_interactive_response)) {
    ...
    {
        if (strlen($this->module->last_interactive_response)) {
            $response = $this->module->last_interactive_response;
        } else {
        ...
        switch ($type) {
            case NET_SSH2_MSG_USERAUTH_INFO_REQUEST:
                ...
                if (strlen($this->module->last_interactive_response)) {
                    $this->module->last_interactive_response = '';
                } else {
                    $this->updateLogHistory('UNKNOWN (60)', 'NET_SSH2_MSG_USERAUTH_INFO_REQUEST');
                }

                if (!count($responses) && $num_prompts) {
                    $this->module->last_interactive_response = $orig;
                    return false;
                }
    ...
    private function get_binary_packet($skip_channel_filter = false)
    {
        ...
        if ($this->decrypt) {
            switch ($this->decrypt->name) {
                ...
                case 'chacha20-poly1305@openssh.com':
                    ...
                    $this->module->lengthDecrypt->setNonce($nonce);
                    $temp = $this->module->lengthDecrypt->decrypt($aad = Strings::shift($raw, 4));
                    extract(unpack('Npacket_length', $temp));
                    /**
                     * @var integer $packet_length
                     */
        ...
        if (defined('NET_SSH2_LOGGING')) {
            ...
            $message_number = '<- ' . $message_number .
                              ' (since last: ' . round($current - $this->module->last_packet, 4) . ', network: ' . round($stop - $start, 4) . 's)';
            $this->append_log($message_number, $payload);
            $this->module->last_packet = $current;
        }
        ...
        if ($this->encrypt) {
            switch ($this->encrypt->name) {
                ...
                case 'chacha20-poly1305@openssh.com':
                    ...
                    $this->module->lengthEncrypt->setNonce($nonce);

                    $length = $this->module->lengthEncrypt->encrypt($packet & "\xFF\xFF\xFF\xFF");
    ...
    private function append_log($message_number, $message)
    {
        ...
        switch (NET_SSH2_LOGGING) {
            ...
            case self::LOG_COMPLEX:
                $this->message_number_log[] = $message_number;
                $this->module->log_size+= strlen($message);
                $this->message_log[] = $message;
                while ($this->module->log_size > self::LOG_MAX_SIZE) {
                    $this->module->log_size-= strlen(array_shift($this->message_log));
    ...
    protected function format_log($message_log, $message_number_log)
    {
        ...
        for ($i = 0; $i < count($message_log); $i++) {
            ...
            do {
                ...
                $fragment = Strings::shift($current_log, $this->module->log_short_width);
                $hex = substr(preg_replace_callback('#.#s', function ($matches) {
                    return $this->module->log_boundary . str_pad(dechex(ord($matches[0])), 2, '0', STR_PAD_LEFT);
                }, $fragment), strlen($this->module->log_boundary));
                // replace non ASCII printable characters with dots
                // http://en.wikipedia.org/wiki/ASCII#ASCII_printable_characters
                // also replace < with a . since < messes up the output on web browsers
                $raw = preg_replace('#[^\x20-\x7E]|<#', '.', $fragment);
                $output.= str_pad($hex, $this->module->log_long_width - $this->module->log_short_width, ' ') . $raw . "\r\n";
                $j++;
