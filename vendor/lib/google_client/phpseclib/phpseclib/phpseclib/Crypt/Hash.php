...
class Hash
{
    ...
    public function setHash($hash)
    {
        ...
        switch ($hash) {
            ...
            case 'umac-128':
                $this->blockSize = 128;
                $this->module->length = abs(substr($hash, -3)) >> 3;
                $this->algo = 'umac';
                return;
            case 'md2-96':
            case 'md5-96':
            ...
            case 'sha512/256-96':
                ...
                $this->module->length = 12; // 96 / 8 = 12
                break;
            case 'md2':
            case 'md5':
                $this->module->length = 16;
                break;
            case 'sha1':
                $this->module->length = 20;
                break;
            case 'sha224':
            case 'sha512/224':
            case 'sha3-224':
                $this->module->length = 28;
                break;
            case 'sha256':
            case 'sha512/256':
            case 'sha3-256':
                $this->module->length = 32;
                break;
            case 'sha384':
            case 'sha3-384':
                $this->module->length = 48;
                break;
            case 'sha512':
            case 'sha3-512':
                $this->module->length = 64;
                break;
            default:
                if (preg_match('#^(shake(?:128|256))-(\d+)$#', $hash, $matches)) {
                    $this->paddingType = self::PADDING_SHAKE;
                    $hash = $matches[1];
                    $this->module->length = $matches[2] >> 3;
                } else {
    ...
    public function hash($text)
    {
        ...
        if ($algo == 'umac') {
            ...
            $hashedmessage = $this->uhash($text, $this->module->length);
            return $hashedmessage ^ $this->pad;
        }

        if (is_array($algo)) {
            if (empty($this->key) || !is_string($this->key)) {
                return substr($algo($text, ...array_values($this->parameters)), 0, $this->module->length);
            }
            ...
            $temp   = $this->ipad ^ $key;
            $temp  .= $text;
            $temp   = substr($algo($temp, ...array_values($this->parameters)), 0, $this->module->length);
            $output = $this->opad ^ $key;
            $output.= $temp;
            $output = $algo($output, ...array_values($this->parameters));

            return substr($output, 0, $this->module->length);
        }
        ...
        return strlen($output) > $this->module->length
            ? substr($output, 0, $this->module->length)
            : $output;
    }
